...
public enum FactionsGroup {
    ...
    public static List<@NonNull FactionsGroup> fromTiers(List<GroupTier> tiers) {
        List<@NonNull FactionsGroup> groups = new ArrayList<>();
        FactionsGroup[] factionsGroups = FactionsGroup.values();
        for (FactionsGroup factionsGroup : factionsGroups) {
    ...
    public static FactionsGroup fromTier(GroupTier groupTier) {
        FactionsGroup[] groups = FactionsGroup.values();
        for (FactionsGroup group : groups) {
    ...
    public static FactionsGroup fromTierId(int id) {
        FactionsGroup[] groups = FactionsGroup.values();
        for (FactionsGroup group : groups) {
    ...
    public static FactionsGroup getHighestPriority(List<FactionsGroup> groups) {
        FactionsGroup highestPriority = FactionsGroup.MORTAL;
        for (FactionsGroup factionsGroup : groups) {
    ...
    public static void addGroupsFromTiers(@NonNull Player player, List<@NonNull GroupTier> tiers) {
        List<FactionsGroup> factionsGroups = fromTiers(tiers);
        ...
        if (user != null) {
            ...
            FactionsGroup finalGroup = FactionsGroup.getHighestPriority(factionsGroups);
    ...
    @NonNull
    public static FactionsGroup getGroupFor(Player player){
        try {
            LuckPerms luckPerms = PrimeFactions.getInstance().getLuckPerms();
            User user = luckPerms.getUserManager().getUser(player.getUniqueId());
            if (user != null) {
                for (FactionsGroup factionsGroup : values()) {
                    if (!factionsGroup.equals(FactionsGroup.MORTAL) && user.getPrimaryGroup().equals(factionsGroup.getName())) {
                        return factionsGroup;
                    }
                }
            }
            return FactionsGroup.MORTAL;
        } catch (Exception e) {
